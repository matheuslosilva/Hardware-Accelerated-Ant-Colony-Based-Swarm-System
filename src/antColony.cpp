...
AntColony::AntColony(int numberAnts, float x, float y, float newSize, int newid, int explorerPheromoneIntensity, int nestCarrierPheromoneIntensity, int carrierPheromoneIntensity)
{
    id = newid;
    numberOfAnts = numberAnts;
    posX = x;
    ...
    {	
        //ants.push_back(new Ant((rand()%100-50)/1000.0f, (rand()%100-50)/1000.0f, glm::radians((float)(rand()%360)), 0.02f, 0.00001f, 10.0f, 1));					
                          //          x                                  y                      theta                                  resize velocity  pheromoneIntensity
        ants.push_back(new Ant(posX, posY, glm::radians((float)(rand()%360)), 0.10f, 0.0002f, explorerPheromoneIntensity, nestCarrierPheromoneIntensity, carrierPheromoneIntensity));
