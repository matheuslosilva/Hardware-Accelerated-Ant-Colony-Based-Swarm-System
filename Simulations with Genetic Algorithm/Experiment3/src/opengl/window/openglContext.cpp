...
//###########################################################################################################################
void addRandomFood(OpenglBuffersManager* openglBuffersManager, Environment *environment)
{
    //srand(time(0));

    FoodSourceParameters* newfoodParameters = (FoodSourceParameters*) malloc(sizeof(FoodSourceParameters));
	newfoodParameters->posX = (double) (((rand() % 500) + 500) / 1000.0) * 0.2;
	newfoodParameters->posY = (double) (((rand() % 500) + 500) / 1000.0) * 0.2;

    if(rand() & 1)  newfoodParameters->posX *= -1;
    if(rand() & 1)  newfoodParameters->posY *= -1;

	newfoodParameters->size = 0.006;
	newfoodParameters->foodAmount = 10000;
    environment->parameterAssigner->foodParameters.push_back(newfoodParameters);

    int sz = (int) environment->parameterAssigner->foodParameters.size();
    environment->createFoodSource(sz - 1, openglBuffersManager);
}

void deleteRandomFood(OpenglBuffersManager* openglBuffersManager, Environment *environment)
{
    //srand(time(0));
    int sz = (int) environment->foods.size();
    
    if(sz <= 1) return;
    
    //int id = rand() % sz; pegar uma fonte de comida aleatoria para ser deletada
    int id = 0; //  pegar a fonte de comida mais antiga para ser deletada

    swap(environment->foods[id], environment->foods.back());
    swap(openglBuffersManager->foodsTransformationMatrices[id], openglBuffersManager->foodsTransformationMatrices.back());

    environment->foods.pop_back();
    environment->numberOfFoods--;
    openglBuffersManager->foodsTransformationMatrices.pop_back();
    openglBuffersManager->updateBuffer(openglBuffersManager->foodsTransformationMatricesVBO, environment->numberOfFoods, openglBuffersManager->foodsTransformationMatrices, GL_STATIC_DRAW);
}
//###########################################################################################################################
        ...
        switch (userInterface->stateSimulation)
        {
            ...
            case RUNNING:
                ...
                {                    
                    ...
                    //  Inserindo nova fonte de comida
                    ...
                    if(frameCounter % 3000 == 0) {
                        addRandomFood(openglBuffersManager, environment);
                    }

                    if(frameCounter % 3000 == 0) {
                        deleteRandomFood(openglBuffersManager, environment);
                    }
